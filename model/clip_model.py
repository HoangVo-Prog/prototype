...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor, average_attn_weights=True):
        self.attn_mask = self.attn_mask.to(dtype=x.dtype, device=x.device) if self.attn_mask is not None else None
        return self.attn(x, x, x, need_weights=True, attn_mask=self.attn_mask, average_attn_weights=average_attn_weights)

    def forward(self, inputs, average_attn_weights=True):
        # x = x + self.attention(self.ln_1(x))
        x = inputs[0] # [x]: round1, [x, atten_weight]: round2,...
        atten, atten_weight = self.attention(self.ln_1(x), average_attn_weights=average_attn_weights)
        x = x + atten
...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, return_all: bool = False, average_attn_weights=True):
        if return_all:
            attns_per_block = []
        for blk in self.resblocks:
            x = blk(x, average_attn_weights=average_attn_weights)
            if return_all:
                attns_per_block.append(x[1])
        if return_all:
            return (x, attns_per_block) # [x: (last_layer_output, last_layer_attn), attns_per_block: list of 12 layers attn] 
        else:
            return x 
...
class CLIP(nn.Module):
    ...
    def encode_text_all_atten(self, text, average_attn_weights=True):
        x = self.token_embedding(text).type(self.dtype)  # [batch_size, n_ctx, d_model]
        x = x + self.positional_embedding.type(self.dtype)
        x = x.permute(1, 0, 2)  # NLD -> LND
        outputs = self.transformer.forward([x], return_all=True, average_attn_weights=average_attn_weights)
        x = outputs[0][0]
        attens = outputs[1]
        x = x.permute(1, 0, 2)  # LND -> NLD
        x = self.ln_final(x).type(self.dtype)
        t_feats = x @ self.text_projection
        
        return t_feats, attens # (B, 77, 512), list of 12 layers attn
    
    def encode_image_all_atten(self, image, average_attn_weights=True):
        ...
        x = x.permute(1, 0, 2)  # NLD -> LND
        outputs = self.visual.transformer.forward([x], return_all=True, average_attn_weights=average_attn_weights)
        x = outputs[0][0]
        attens = outputs[1]
        x = x.permute(1, 0, 2)  # LND -> NLD
        ...
        return x, attens # (B, 193, 512), list of 12 layers attn

    def forward(self, image, text, return_all: bool = False, average_attn_weights=True):
        if return_all:
            image_features, atten_i = self.encode_image_all_atten(image, average_attn_weights=average_attn_weights)
            text_features, atten_t = self.encode_text_all_atten(text, average_attn_weights=average_attn_weights)
        else:
